...
<template>
  <div>
    ...
    <template #default="scope">{{ formatBoolean(scope.row.feeType) }}</template>
</el-table-column>
            <el-table-column align="left" label="1-小柜子 2-大柜子" prop="lockerType" width="120">
    <template #default="scope">{{ formatBoolean(scope.row.lockerType) }}</template>
</el-table-column>
    ...
    <template #default="scope">{{ formatBoolean(scope.row.isDepositEnabled) }}</template>
</el-table-column>
            <el-table-column align="left" label="是否启用预付" prop="isAdvancePay" width="120">
    <template #default="scope">{{ formatBoolean(scope.row.isAdvancePay) }}</template>
</el-table-column>
   ...
   <template #default="scope">{{ formatDate(scope.row.updatedAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateLockerPricingRulesFunc(scope.row)">编辑</el-button>
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    if (!valid) return
    page.value = 1
    if (searchInfo.value.feeType === ""){
        searchInfo.value.feeType=null
    }
    if (searchInfo.value.lockerType === ""){
        searchInfo.value.lockerType=null
    }
    if (searchInfo.value.isDepositEnabled === ""){
        searchInfo.value.isDepositEnabled=null
    }
    if (searchInfo.value.isAdvancePay === ""){
        searchInfo.value.isAdvancePay=null
    }
