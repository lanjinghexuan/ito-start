  ...
  <el-input v-model="searchInfo.title" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate v-auth="btnAuth.exportTemplate" template-id="ito_user_LockerOrders" />
            <ExportExcel v-auth="btnAuth.exportExcel" template-id="ito_user_LockerOrders" filterDeleted/>
            <ImportExcel v-auth="btnAuth.importExcel" template-id="ito_user_LockerOrders" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
   ...
   <template #default="scope">{{ formatDate(scope.row.startTime) }}</template>
            ...
            <el-table-column align="left" label="优惠金额" prop="discount" width="120" />

            <el-table-column align="left" label="实付金额" prop="amountPaid" width="120" />

            <el-table-column align="left" label="寄存网点名称" prop="storageLocationName" width="120" />
   ...
   <template #default="scope">{{ formatDate(scope.row.updateTime) }}</template>
    ...
    {{ filterDict(scope.row.depositStatus,depositStatusOptions) }}
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateLockerOrdersFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.amountPaid }}
</el-descriptions-item>
                    <el-descriptions-item label="寄存网点名称">
    {{ detailForm.storageLocationName }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getLockerOrdersList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteLockerOrdersByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateLockerOrdersFunc = async(row) => {
    const res = await findLockerOrders({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteLockerOrdersFunc = async (row) => {
    const res = await deleteLockerOrders({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
