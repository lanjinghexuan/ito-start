...
type LockerPricingRules struct {
  ...
  FeeType  *string `json:"feeType" form:"feeType" gorm:"comment:1-计时收费 2-按日收费;column:fee_type;" binding:"required"`  //收费类型
  LockerType  *string `json:"lockerType" form:"lockerType" gorm:"comment:1-小柜子 2-大柜子;column:locker_type;" binding:"required"`  //柜子类型
  FreeDuration  *float64 `json:"freeDuration" form:"freeDuration" gorm:"comment:免费时长(小时);column:free_duration;size:5;" binding:"required"`  //免费时长
  ...
  Status  *string `json:"status" form:"status" gorm:"comment:1-生效 0-失效;column:status;"`  //状态
