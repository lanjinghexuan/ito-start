...
type LockerOrders struct {
    global.GVA_MODEL
  OrderNumber  *string `json:"orderNumber" form:"orderNumber" gorm:"comment:业务订单号（唯一标识）;column:order_number;size:200;" binding:"required"`  //业务订单号
  UserId  *int `json:"userId" form:"userId" gorm:"comment:用户ID（关联用户表）;column:user_id;size:20;" binding:"required"`  //用户ID
  StartTime  *time.Time `json:"startTime" form:"startTime" gorm:"comment:寄存开始时间;column:start_time;"`  //寄存开始时间
  ScheduledDuration  *int `json:"scheduledDuration" form:"scheduledDuration" gorm:"comment:计划寄存时长（小时）;column:scheduled_duration;size:10;" binding:"required"`  //计划寄存时长
  ActualDuration  *int `json:"actualDuration" form:"actualDuration" gorm:"comment:实际寄存时长（小时）;column:actual_duration;size:10;"`  //实际寄存时长
  ...
  Status  *string `json:"status" form:"status" gorm:"comment:订单状态：1-待支付、2-寄存中、3-已完成、4-已取消、5-超时、6-异常;column:status;" binding:"required"`  //订单状态
  ...
  DepositStatus  *string `json:"depositStatus" form:"depositStatus" gorm:"comment:押金状态：1-已支付、2-已退还、3-已扣除;column:deposit_status;" binding:"required"`  //押金状态
  LockerPointId  *int `json:"lockerPointId" form:"lockerPointId" gorm:"comment:寄存点id;column:locker_point_id;size:10;"`  //寄存点id
  Title  *string `json:"title" form:"title" gorm:"column:title;size:50;" binding:"required"`  //标题
